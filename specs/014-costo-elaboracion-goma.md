...
ajustes clave→valor por usuario, donde viven los costos de los tres tamaños y donde caben los
...
son tres cosas y solo tres: la palabra "goma", los nombres Chica/Mediana/Grande, y los montos
$6.00 / $10.00 / $20.00.
...
`enum` de PHP backed por `string`, con tres casos y **ningún caso para "sin goma"**:
...
- **No tiene endpoint de catálogo**: sus tres opciones, sus etiquetas y sus medidas de referencia se
  sirven embebidas en el frontend, igual que las cuatro opciones de `objeto_imp` en 006. El SAT no
  publica un catálogo de tamaños de goma.

### Cambios sobre `Articulo` (extiende 006, 009 y 011)

- **Nueva columna `tamano_goma`**: string(10), **nullable**. `NULL` = el artículo no lleva goma.
  Valores admitidos: `chica`, `mediana`, `grande`.
...
Cambiar cualquiera de los tres `costo_goma_*` dispara el recálculo de los artículos cuyo
...
- `GET /api/v1/configuracion` — devuelve **todas** las claves declaradas en el enum con su valor
  ...
  Nunca devuelve un objeto incompleto: el frontend no tiene que saber qué es un valor por defecto.
...
- `GET /api/v1/configuracion/impacto-precios?costo_goma_chica=&costo_goma_mediana=&costo_goma_grande=`
...
- `tamano_goma` en `Articulo`: **nullable**, y si viene, uno de `chica`, `mediana`, `grande`
  (`Rule::enum(TamanoGoma::class)`). Cadena vacía se normaliza a `NULL` antes de validar, para que
  un `<select>` sin selección y una ausencia de campo se comporten igual.
- `PUT /api/v1/configuracion`: cada clave enviada se valida con la regla que declara su caso del
  enum. Los tres costos de goma son numéricos, **mayores o iguales a 0** y de máximo 2 decimales.
...
- Los valores admitidos son `chica`, `mediana` y `grande`, **insensibles a mayúsculas y a espacios
  alrededor** (`Grande `, `GRANDE` y `grande` son la misma cosa). Cualquier otro texto rechaza la
  fila con su motivo, sin abortar el archivo, igual que el resto de errores de 006.
...
- **Configuración**: `GET` devuelve las tres claves aunque no exista ninguna fila; `PUT` con una
  clave desconocida devuelve `422`; `PUT` parcial deja intactas las claves ausentes; un costo
  negativo se rechaza; `0.00` se acepta; los ajustes de un usuario no son visibles ni modificables
  por otro.
- **CSV**: importar con la celda de tamaño vacía deja el artículo sin goma; con `Grande ` (con
  espacio y mayúscula) lo deja en grande; con `enorme` rechaza la fila reportando número y motivo;
  exportar produce las 8 columnas y el archivo resultante es reimportable sin pérdida del tamaño.
...
Primera pantalla de configuración del sistema. Una sección, **Costos de elaboración**, con los tres
`Input` numéricos:
...
- **Diálogo de confirmación antes de guardar** cuando cambia alguno de los tres costos: antes de
...
- **Selector `Tamaño de goma`** (`Select` simple, opcional), con cuatro opciones donde la primera es
...
- **Categorías de tamaño administrables por el usuario**: son tres, fijas, definidas en el código.
  No se pueden crear, renombrar, reordenar ni eliminar desde la aplicación.
...
- **Costos de goma distintos por proveedor, catálogo o cliente**: los tres costos son globales del
  usuario.
...
- **Multi-moneda**: los tres costos son en MXN, sin IVA.
...
8. `/configuracion` muestra los tres costos con valores reales desde el primer día, aun sin haber
...
11. Cambiar `costo_goma_mediana` recalcula el costo y el precio de venta **solo** de los artículos
    con goma mediana; los chicos, los grandes y los que no llevan goma conservan su precio.
...
18. Importar un CSV con las 8 columnas da de alta los artículos con su goma y su precio ya
    ...
    deja en grande, y un valor no reconocido rechaza esa fila reportando número y motivo sin abortar
    el archivo.
...
3. Los tamaños son tres valores fijos del sistema (Chica, Mediana, Grande). El usuario no puede
   crear, renombrar ni eliminar categorías.
...
7. Nace una pantalla de Configuración —la primera del sistema— donde se editan los tres costos,
   precargados en $6.00 / $10.00 / $20.00.
...
24. **(Adición técnica)** El artículo guarda una palabra corta (`chica`/`mediana`/`grande`) validada
    contra una lista cerrada, y `NULL` significa "sin goma". Mismo patrón que `objeto_imp` en 006 y
