...
- **Inventario y existencias**: marcar una orden como `recibida` no suma stock en ningún lado, porque
  el sistema no lleva inventario.
...
12. Una orden `pagada` puede marcarse como `recibida` manualmente; no puede marcarse así desde ningún
    otro estado, y hacerlo no afecta existencias de ningún tipo.
...
14. `recibida` es una marca manual del usuario, solo alcanzable desde `pagada`, sin validación de
    cantidades ni efecto sobre inventario.
...
29. No hay inventario ni existencias: marcar "recibida" no suma stock.
