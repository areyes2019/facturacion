...
De nueve elementos sueltos se pasa a cuatro grupos más el título:
...
| **Cerrar sesión** | `ArrowRightStartOnRectangleIcon` | — |
...
- **Botón "Cerrar sesión"**: sin cambios respecto del estado actual (posición, variante `outline`,
  ícono y comportamiento).
...
`ArrowsRightLeftIcon`, Saldos `ScaleIcon`.
...
Los desplegables usan `NavigationMenu` de shadcn-vue / Reka UI, que hoy **no** está en
...
- Abrir un grupo cierra el que estuviera abierto: nunca hay dos desplegables abiertos a la vez.
...
- El título "Facturación" y "Cerrar sesión" permanecen visibles en la barra móvil.
...
menú **declara a qué grupo pertenece** (por ejemplo, "se agrega Recibos al grupo Ventas") y se
implementa como una entrada en `src/config/navegacion.ts`.

Ninguna spec agrega enlaces sueltos al header. Si una funcionalidad no encaja en ningún grupo
existente, eso es señal de que la estructura de grupos debe revisarse en esta spec, no de que haya
que sumar un botón más a la barra.
...
- Cambios en el botón "Cerrar sesión" (menú de usuario, avatar, perfil): quedan para una historia
  futura.
...
- **El botón "Cerrar sesión" oculta su texto por debajo de `sm`** (`hidden sm:inline`), quedando
  solo el ícono, para dejar lugar al hamburguesa en 375px. La asunción 15 se refiere a su
  posición y comportamiento, que no cambian.
...
1. La barra superior muestra el título "Facturación" y cuatro grupos: Ventas, Compras, Inventario
   y Contabilidad. No quedan enlaces planos de sección en el header.
...
15. El botón "Cerrar sesión" conserva su posición y comportamiento actuales.
